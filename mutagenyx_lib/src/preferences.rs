...
impl TryFrom<Value> for PreferenceValue {
    ...
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        ...
        Err(MutagenyxError::UnrecognizedJSON(json_text))
    }
}
...
impl TryFrom<Value> for Preferences {
    ...
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        if !value.is_object() {
            let json_text = format!("{value}");
            return Err(MutagenyxError::UnrecognizedJSON(json_text));
        }
...
mod tests {
    ...
    fn test_json_into_preference_value() {
        let ipvalue: Result<PreferenceValue, MutagenyxError> = Value::from(10).try_into();
        assert_eq!(ipvalue.unwrap(), PreferenceValue::Int(10));

        let fpvalue: Result<PreferenceValue, MutagenyxError> = Value::from(3.4).try_into();
        assert_eq!(fpvalue.unwrap(), PreferenceValue::Float(3.4));

        let spvalue: Result<PreferenceValue, MutagenyxError> = Value::from("foo").try_into();
        assert_eq!(
            spvalue.unwrap(),
            PreferenceValue::String(String::from("foo"))
        );

        let bpvalue: Result<PreferenceValue, MutagenyxError> = Value::from(true).try_into();
        ...
        let opvalue: Result<PreferenceValue, MutagenyxError> = jobject_value.try_into();
        ...
        let apvalue: Result<PreferenceValue, MutagenyxError> = jarray_value.try_into();
        assert_eq!(apvalue.unwrap(), actual_array);

        let npvalue: Result<PreferenceValue, MutagenyxError> = Value::Null.try_into();
