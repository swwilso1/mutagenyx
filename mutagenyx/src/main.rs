...
fn main() {
    let _ = env_logger::builder()
        ...
        .try_init();
    match MutagenyxCommand::parse() {
        MutagenyxCommand::Mutate(params) => {
            if let Err(e) = generate_mutants(params) {
                println!("Unable to generate mutations: {}", e);
            }
        }
        MutagenyxCommand::PrettyPrint(params) => {
            pretty_print_files(params);
        }
        MutagenyxCommand::Algorithms(params) => {
