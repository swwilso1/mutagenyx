...
command line flag. Mutagenyx will also use a custom compiler specification in a `.morph` configuration file.  Set the
...
The `--save-config-files` flag instructs Mutagenyx to generate a `.morph` [configuration file](#configuration-files) for
...
configuration files.  Mutagenyx configuration files have the file extension `.morph`.  Mutagenyx configuration files
use JSON formatting.  <mark>Important</mark>: Mutagenyx will reject any configuration file that does not have the `.morph` extension.
...
of filenames.  Mutagenyx does not support configuration files (`.morph`) files in this array of input files.
